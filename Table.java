        ...
        implements Serializable
{
    ...
    public Table select (KeyType keyVal)
    {
        ...
        for (Comparable[] tuple : tuples) {
            KeyType tupleKey = new KeyType(tuple[0]);
...
*/
    public Table join(String attributes1, String attributes2, Table table2) {
        ...
        int attrIndex1 = Arrays.asList(attribute).indexOf(attributes1);
        int attrIndex2 = Arrays.asList(table2.attribute).indexOf(attributes2);

        List<Comparable[]> newRows = new ArrayList<>();

        // Iterate over all tuples in the current table
        for (Comparable[] tuple1 : tuples) {
            // Iterate over all tuples in the second table
            for (Comparable[] tuple2 : table2.tuples) {
                if (tuple1[attrIndex1].equals(tuple2[attrIndex2])) {
                    // If the join condition is met, concatenate the tuples
                    Comparable[] joinedRow = new Comparable[tuple1.length + tuple2.length];
                    System.arraycopy(tuple1, 0, joinedRow, 0, tuple1.length);
                    System.arraycopy(tuple2, 0, joinedRow, tuple1.length, tuple2.length);
                    newRows.add(joinedRow);
                }
            }
        }

        // Combine attribute arrays and domain arrays
        String[] newAttributes = new String[attribute.length + table2.attribute.length];
        Class[] newDomains = new Class[domain.length + table2.domain.length];
        System.arraycopy(attribute, 0, newAttributes, 0, attribute.length);
        System.arraycopy(table2.attribute, 0, newAttributes, attribute.length, table2.attribute.length);
        System.arraycopy(domain, 0, newDomains, 0, domain.length);
        System.arraycopy(table2.domain, 0, newDomains, domain.length, table2.domain.length);

        return new Table(name + "_joined_" + table2.name, newAttributes, newDomains, key, newRows);
    }
    /************************************************************************************
     * Join this table and table2 by performing an "equi-join".  Tuples from both tables
     * are compared requiring attributes1 to equal attributes2.  Disambiguate attribute
    ...
    public Table join(Table table2) {
        ...
        // check what is in both tables, add all matches in table1 to match1, add all matches in table2 to match2
        for (int i = 0; i < this.attribute.length; i++) {
            for (int j = 0; j < table2.attribute.length; j++) {
                if ((table2.attribute[j]).equals(this.attribute[i])) {
                    match1.add(i); // add to the array list
                    match2.add(j);
                }
            }
        }

        // finds intersection of both tables then adds the row to the result table
        for (int i = 0; i < this.tuples.size(); i++) {
            for (int j = 0; j < table2.tuples.size(); j++) {
                int matched = 0;
                for (int a = 0; a < match1.size(); a++) {
                    if (this.tuples.get(i)[match1.get(a)].equals(table2.tuples.get(j)[match2.get(a)])) {
                        matched++;
                    }
                }
                if (matched == match1.size()) {
                    Comparable[] table2tuple = new Comparable[table2.attribute.length - match1.size()];
                    int table2columnCounter = 0;
                    if (table2tuple.length != 0) {
                        for (int b = 0; b < table2.attribute.length; b++) {
                            if (!match2.contains(b)) {
                                table2tuple[table2columnCounter] = table2.tuples.get(j)[b];
                                table2columnCounter++;
                            }
                        }
                    }
                    rows.add(ArrayUtil.concat(this.tuples.get(i), table2tuple));
                }
            }
        }

        // adds 2 to the end of duplicate names in 2nd table so they aren't used
        int numDuplicates = 0;
        for (int i = 0; i < this.attribute.length; i++) {
            for (int j = 0; j < table2.attribute.length; j++) {
        ...
        // only adds new attributes to arraylist by checking for 2 at the end indicating duplicate
        String[] newAttribute = new String[table2.attribute.length - numDuplicates];
        int attributesToAdd = 0;
        for (int i = 0; i < table2.attribute.length; i++) {
            char charCheck = table2.attribute[i].charAt(table2.attribute[i].length() - 1);
            if (charCheck != '2') {
                newAttribute[attributesToAdd] = table2.attribute[i];
                attributesToAdd++;
            }
        }
        ...
        return new Table(name + count++, ArrayUtil.concat(attribute, newAttribute),
                ArrayUtil.concat(domain, Arrays.copyOf(table2.domain, newAttribute.length)), key, rows);
    } // join
